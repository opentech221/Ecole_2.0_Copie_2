...
run_query() {
  ...
  echo "$title"
  echo "============================================================"
  npx supabase db query "$TARGET_FLAG" "$sql"
}

run_query "1) Required tables" "
...
run_query "3) Critical foreign keys (delete behavior)" "
with fk as (
  select
    conname,
...
echo
echo "Checklist finished in mode: $MODE"
