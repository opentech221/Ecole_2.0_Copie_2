...
create view public.programme_flat_os_v as
select
  n.id               as niveau_id,
  n.nom              as niveau_nom,
  d.id               as domaine_id,
...
join public.competences_base cb     on cb.activite_id    = a.id
join public.paliers p               on p.competence_id   = cb.id
join public.objectifs_apprentissage oa on oa.palier_id   = p.id
join public.objectifs_specifiques os   on os.oa_id       = oa.id
join public.contenus c              on c.os_id           = os.id;
